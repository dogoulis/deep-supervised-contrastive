...
class GANDataset(pl.LightningDataModule):
    def __init__(self, dataset_path, batch_size=32, num_workers=8, transformrs=None, root_dir=None, csv_names=None):
        super().__init__()
    ...
    def setup(self, stage=None):
        ...
        if stage in (None, 'fit'):
            self.train_dataset = dataset2(dataset_path=self.csv_names[0], root_dir=self.root_dir, transforms=self.transforms)
            self.val_dataset = dataset2(dataset_path=self.csv_names[1], root_dir=self.root_dir, transforms=self.transforms)
        if stage in (None, 'test'):
            self.test_dataset = dataset2(dataset_path=self.csv_names[2], root_dir=self.root_dir, transforms=self.transforms)
