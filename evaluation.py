...
# experiment config
config = {"projector": [2048] + [8196, 8196, 8196]}
...
parser.add_argument(
    "--test_dir",
    type=str,
    metavar="testing-directory",
    help="Directory of the testing csv",
)

parser.add_argument("--id", type=int, metavar="id", help="id of the test")

parser.add_argument(
    "--weights_dir", type=str, metavar="weights_dir", help="Directory of weights"
)
...
def main():
    ...
    print(args.name)
    wandb.init(
        project=args.project_name, name=args.name, config=vars(args), group=args.group
    )
    ...
    # define test dataset:
    test_dataset = pytorch_dataset.dataset2(args.dataset_dir, args.test_dir, transforms)
